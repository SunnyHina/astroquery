...
and FITS. Its cone service returns VOTable even when CSV is requested,
with names such as ``catalogue_obsid``.
...
A legacy VOTable can encode a missing integer as an unmasked zero. If the
standard parser does not preserve its mask, the client raises
``TableParseError``; real zeros remain valid. Malformed CSV and truncated
VOTable values are not guessed or repaired.
...
.. code-block:: python
   ...
   while True:
       ...
       if len(matches) == 0:
           break
       matches.write(f'lamost-page-{page}.ecsv', format='ascii.ecsv')
       page += 1
...
.. code-block:: python
   ...
   wavelength, flux = parse_lrs_spectrum('spectrum.fits')
   Table({'wavelength': wavelength, 'flux': flux}).write(
       'spectrum.csv', format='ascii.csv')
...
does not disappear from the sample:
...
.. code-block:: python
   ...
   spectra, rows = [], []
   for filename in ['first.fits', 'second.fits']:
       try:
...
.. code-block:: python
   ...
   fig, ax = plt.subplots()
   for name, spectrum in parse_mrs_spectrum('spectrum.fits').items():
