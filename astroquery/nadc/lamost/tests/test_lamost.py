...
class TestLamost:
    ...
    def test_query_catalog_rejects_incomplete_cone_matches(self, monkeypatch, method):
        request = Mock()
        ...
        with pytest.raises(RemoteServiceError, match='query_region.*csv'):
            getattr(LamostClass(), method)(*args, columns=['obsid'], max_rows=1, **kwargs)
    ...
    def test_query_spectra_builds_quality_payload(self):
        ...
        assert payload['showcol'] == ['obsid', 'ra', 'dec', 'snrg', 'teff', 'logg', 'feh']
        ...
        assert payload['column_constraints'] == [
            {'column_name': 'snrg', 'operation': 'greaterequal', 'constraint': '20'},
    ...
    @pytest.mark.parametrize('method, endpoint, position', [
        ('region', 'conesearch', {'ra': 10., 'dec': 40., 'sr': .2}),
        ('ssap', 'ssap', {'pos': '10.0,40.0', 'size': .2}),
    ])
    ...
    def test_request_query_format(self, patch_request, mock_votable_response, mock_json_response,
                                  method, endpoint, position, output_format):
        ...
        result = getattr(client, '_request_query_' + method)(
            SkyCoord(10, 40, unit='deg'), radius=.2 * u.deg, output_format=output_format, cache=False,
        )
        assert result is response
        request.assert_called_once_with(
            'GET', f'https://www.lamost.org/openapi/dr10/v2.0/lrs/voservice/{endpoint}',
            params={**position, 'output.fmt': output_format, 'token': 'query-token'},
            json=None, timeout=60, cache=False, stream=False,
        )

    @pytest.mark.parametrize('method', ['query_region', 'query_ssap'])
    def test_query_fmt_alias_warns(self, method):
        with pytest.warns(DeprecationWarning, match='deprecated'):
            payload = getattr(LamostClass(), method)(
                SkyCoord(10, 40, unit='deg'), .2 * u.deg, fmt='json', get_query_payload=True,
            )
        assert payload['output.fmt'] == 'json'
...
class TestLamostDiagnostics:
    ...
    @pytest.mark.parametrize('method, args, kwargs', [
        ...
        ('query_ssap', (), {'coordinates': SkyCoord(10, 40, unit='deg'), 'radius': '5 arcsec'}),
...
class TestLamostDataDiscovery:
    ...
    def test_get_tap_url(self, patch_request):
        tap_info = {'tap_url': 'https://www.lamost.org/dr10/v2.0/tap', 'tap_version': '1.1'}
        request = patch_request(create_mock_response(json_data=tap_info))
        assert LamostClass(token='tap-token').get_tap_url(cache=False) == tap_info
        request.assert_called_once_with(
    ...
    def test_get_unique_id(self, patch_request, parameters):
        data = {'unique_id': 'U_12345', 'related_obsids': ['101001', '101002', '101003']}
        request = patch_request(create_mock_response(json_data=data))
        result = LamostClass(token='id-token').get_unique_id_and_related_obsids(**parameters, cache=False)
        assert result['unique_id'] == data['unique_id']
        assert result['related_obsids'] == data['related_obsids']
        request.assert_called_once_with(
