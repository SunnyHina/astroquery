...
def _response_json_payload(response) -> Optional[Mapping[str, Any]]:
    ...
    if "json" not in content_type:
        ...
        if content is False:
            # ``requests.Response`` uses False for an unconsumed streaming
            # body. Do not materialize a potentially large data product merely
            # to test whether it looks like JSON.
            return None
        if content is None:
            content = getattr(response, "content", None)
        if not isinstance(content, bytes) or not content.lstrip().startswith((b"{", b"[")):
            return None
...
def _oauth_redirect_url(response) -> Optional[str]:
    candidates = []
