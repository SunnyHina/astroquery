...
def metadata_fields(table):
    """Read a single metadata record, including the DR7 display response."""
    if len(table) != 1:
    ...
    fields = {}
    for item in table['response'][0]:
        if item['what'] in fields:
            raise ValueError('Duplicate spectrum metadata field.')
        fields[item['what']] = item['data']
    return fields
